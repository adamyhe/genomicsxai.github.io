...
**Training equivalence.** Inference equivalence doesn't guarantee the training loop itself matches — a converter can produce an identical model while the from-scratch training and fine-tuning code silently diverges from Illumina's Keras implementation. Auditing `train.py`/`finetune.py` against Illumina's actual source turned up two real discrepancies since fixed: gradient clipping was clipping one norm across all parameters jointly instead of each parameter independently (matching Keras' `clipnorm` semantics), and `BatchNorm`'s momentum was left at PyTorch's default instead of the value equivalent to Keras' `momentum=0.99`. A dedicated test now runs one training step through numerically identical converted weights in both frameworks and checks agreement on the loss, gradients, AdamW parameter deltas, and BatchNorm running-stat updates. It also surfaces a separate, framework-inherent quirk — Keras' `AdamW` places its `epsilon` term differently than PyTorch's, transiently damping its first ~1,000 optimizer steps more strongly even with a matching `epsilon` — which isn't a porting bug and isn't worth chasing with a custom optimizer. See `notes/implementation.md` and `docs/training.md` in the repo for the full writeup.
...
8. Schreiber, J., Lorbeer, F. K., Heinzl, M., Reiter, F., Rafanel, B., Lu, Y. Y., Stark, A. & Noble, W. S. Programmatic design and editing of cis-regulatory elements. *bioRxiv* (2025). https://doi.org/10.1101/2025.04.22.650035
