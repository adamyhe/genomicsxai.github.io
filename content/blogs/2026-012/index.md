...
By integrating evidence from these and many other assays not discussed here, ENCODE has identified approximately 2.4 million [**candidate cis-regulatory elements (cCREs)**](https://screen.wenglab.org/), in the human genome. A cCRE is a genomic region whose biochemical properties suggest that it may regulate gene expression. These include promoters immediately upstream of genes where transcription begins, enhancers that can increase transcription—sometimes from considerable genomic distances—and [other classes of regulatory elements](https://screen.wenglab.org/about#classifications). The word candidate is important: the biochemical evidence suggests regulatory activity, but most elements have not yet been directly shown to regulate a particular gene.

The consortium recently released a preprint describing the expanded Encyclopedia generated during the fourth and final phase of the project, ENCODE 4.

The ENCODE consortium recently released a preprint describing the entire compendium developed over two decades including many new datasets and derived analysis products from the fourth and final phase of the project [ENCODE 4](https://www.biorxiv.org/content/10.64898/2026.07.06.731365v1).
...
We developed the **BPNet family** of deep learning models to address these questions. These compact, fully convolutional neural networks use dilated residual layers to learn how local DNA sequence gives rise to the regulatory activity measured in each experiment. Rather than simply classifying a genomic region as active or inactive, BPNet models predict both the total amount of activity and the shape of the experimental signal at **base-pair resolution**.  

The ENCODE GRAMMAR model resource contains four related model families:

- **BPNet:** 2,339 TF ChIP-seq model sets spanning 788 transcription-factor targets.
- **ChromBPNet:** 1,512 DNase-seq and ATAC-seq model sets across 408 biosamples.
- **ProCapNet:** 6 PRO-cap model sets that predict transcription-initiation profiles.
...
The trained models are only one component of the resource. We also developed a suite of interpretation methods (discussed below) that reveal the predictive DNA sequence features learned by each model. For every experiment, ENCODE GRAMMAR provides several derived products:
**Predicted regulatory profiles** at base-pair resolution.
**Bias-factorized regulatory profiles** for chromatin-accessibility assays, separating the predicted biological signal from sequence preferences of the Tn5 and DNase I enzymes used in ATAC–seq and DNase–seq, respectively.
**Sequence-contribution maps** estimating how much each DNA base contributes to a model prediction.
**De novo predictive sequence motifs** which are derived from recurrent patterns of high contribution scores with similar sequences across regions with high signal (e.g. peaks)
**MotifCompendium** which is a unified lexicon of non-redundant sequence motifs derived from all ENCODE GRAMMAR models 
**Predictive genomic motif instances** which map high-contribution sequence patterns in regions with high signal to the unified motif lexicon.
Genome-browser tracks that allow predictions, contribution scores, motifs, and motif instances to be explored together at any genomic locus.
